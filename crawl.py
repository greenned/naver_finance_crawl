...
class FinanceCralwer:
    def __init__(self, start_date, end_date, code_list=[], debug=False):
        """
        from_ = "2020-01-01"
        to_ = "2020-01-01"
        ...
        # 날짜형식 초기화
        self.init_date(start_date, end_date)
    ...
    def init_date(self, from_, to_):
        ...
        self.datetime_from = datetime.datetime(
            year=split_from[0], month=split_from[1], day=split_from[2]
        )
        self.datetime_to = datetime.datetime(
            year=split_to[0], month=split_to[1], day=split_to[2]
        )
        self.from_ = datetime.datetime.strftime(
            datetime.datetime(
                year=split_from[0], month=split_from[1], day=split_from[2]
            ),
            "%Y.%m.%d",
        )
        self.to_ = datetime.datetime.strftime(
            datetime.datetime(year=split_to[0], month=split_to[1], day=split_to[2]),
            "%Y.%m.%d",
        )
...
if __name__ == "__main__":
    ...
    t = FinanceCralwer(
        start_date="2020-01-01", end_date="2020-01-31", code_list=code_list, debug=False
    ).main()
    print(t)
